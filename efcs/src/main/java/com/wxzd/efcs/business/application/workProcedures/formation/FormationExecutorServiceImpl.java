 ...
 */
@Service("formationExecutorService")
public class FormationExecutorServiceImpl implements FormationExecutorService<DefaultProcedure, ObjectResult<Instruction>> {
    ...
    private WorkProcedure currentWorkProcedure = WorkProcedure.Formation;
    ...
    public ObjectResult<Instruction> doWorkProcedure(DefaultProcedure procedureInfo) {
        ...
        if (procedureInfo.getCurrentPos().equals(DispatcherConfig.formation_allot_location)) {
...
//                }
            ...
            //化成分库位 入库完成
            //入到化成立库了
            if (procedureInfo.getStorageLocation() != null &&(procedureInfo.getStorageLocation().getX_pos().equals(3)||procedureInfo.getStorageLocation().getX_pos().equals(4))&&
...
//                instructionService.createInstr(instruction);
                ...
                //都不是上面的情况
                //启动自动寻径
                if (!DispatcherConfig.noNeedSendNextPosition.contains(procedureInfo.getCurrentPos())&&ProcedureRouteMatch.getEquipMap().values().contains(procedureInfo.getCurrentPos())) {
