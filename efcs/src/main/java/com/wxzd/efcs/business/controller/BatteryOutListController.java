...
@Controller
@RequestMapping
public class BatteryOutListController {
    ...
    public PageResult<FmBatteryDto> grid(FmBatteryExQuery fmBatteryExQuery){
        return fmBatteryAppService.getFmBatteryOutPaged(fmBatteryExQuery);
    }

    @RequestMapping
    public void detail_view(UUID id,Model model){
        FmBatteryDto fmBatteryDto = fmBatteryAppService.getFmBatteryOutById(id);
