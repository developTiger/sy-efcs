        ...
        initChart: function () {
            ...
            timeChunk(this.$activeContainer.find(".chart-container"), function (obj) {
                var $obj = $(obj),
                    ...
                    drag = dataSet["drag"],
                    _type = type,
                    isStatistics = dataSet["statistics"];
                ...
                var heatmap = new HeatMap({
                    ...
                    statisticsParams: isStatistics === false ? null : {houseId: houseId, type: _type, x: x},
                    events: self.get("legendTrigger") ? events : {},
                    mousedown: self.get("mousedown"),
                    dataKeys:_type == "cw"? ["x_pos","y_pos","z_pos","temperature","loc_is_error"] :["x_pos","y_pos","z_pos","storage_status","loc_is_error"]
                });

                $obj.data("heatmap", heatmap);

                self.trigger("heatmap.init", $obj, heatmap);
            }, 1)()
        },
